...
namespace LatexDrawGraphics {
    ...
    public:
        ...
        void init( std::string unicode_s )
        {
            ...
            _face = _file.face;
            _glyph_index = FT_Get_Char_Index( _face, _codepoint );
            _error = FT_Load_Glyph( _face, _glyph_index, FT_LOAD_NO_SCALE );
        ...
        std::string GlyphName() const
        {
            ...
            FT_Get_Glyph_Name( _face, _glyph_index, glyph_name, 1024 );
            return std::string(glyph_name);
        }
        
        long HorizAdvance() const { return _gm.horiAdvance; }
        long VertAdvance() const  { return _gm.vertAdvance; }
        long Height() const       { return _gm.height; }
        long Width() const        { return _gm.width; }
        long bbHeight() const { return _face->bbox.yMax - _face->bbox.yMin; }
        long bbWidth() const  { return _face->bbox.xMax - _face->bbox.xMin; }
        
        long NPoints() const { return _outline.n_points; }
        long NContours() const { return _outline.n_contours; }
        
        // Find the end points of the contours
        std::string EndPoints() const
        {
            std::stringstream tmp;
            for ( int i = 0 ; i < _outline.n_contours ; i++ ) tmp << " " << _outline.contours[i];
            return tmp.str();
        }
        ...
        void InvertPoints()
        {
            FT_Vector* _points = _outline.points;
        ...
        std::string points()
        {
            FT_Vector* points = _outline.points;
            char* tags = _outline.tags;

            std::stringstream tmp("");

            for ( int i = 0 ; i < _outline.n_points ; i++ ) {
                bool this_is_ctrl_pt = !(tags[i] & 1);
                bool next_is_ctrl_pt = !(tags[(i+1) % _outline.n_points] & 1);
                long x = points[i].x;
                long y = points[i].y;
                long nx = points[(i+1) % _outline.n_points].x;
                long ny = points[(i+1) % _outline.n_points].y;
                int radius = 5;
                if ( i == 0 ) radius = 10;
        ...
        {
            FT_Vector* points = _outline.points;
            short* contours = _outline.contours;
            ...
            for ( int i = 0 ; i < _outline.n_points-1 ; i++ ) {
                ...
                tmp << dash_mod;
                tmp << " d='";
                tmp << " M " << points[i].x << "," << points[i].y;
                tmp << " L " << points[(i+1) % _outline.n_points].x << "," << points[(i+1) % _outline.n_points].y;
                tmp << "\n  '/>";
            }
            return tmp.str();
        }
